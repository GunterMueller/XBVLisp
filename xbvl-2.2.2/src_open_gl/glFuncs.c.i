...
Int gl_vertex()
{
  ...
  if(wobj && isnum(a2) && (dtype = valnb(a2)) > 1 && dtype < 5) {
    ...
    for( ;; ) {
      if(!getNumArgFroma1((char *) data, dtype, GL_FLOAT)) break;
...
Int gl_normal()
{
  ...
  if(wobj) {
    ...
    if(isnum(a2) && (dtype = valnb(a2)) > 1 && dtype < 5) 
      for( ;; ) {
	if(!getNumArgFroma1((char *) data,3,GL_FLOAT)) break;
	glNormal3fv(data);
      }
    a1 = t;
  }
...
{
  float data[4];
  if(!wobj || !getNumArgFroma1((char *) data,4,GL_FLOAT)) {a1=nil; derec;}
...
Int gl_get_real_position() /* (GLgetposition x y z rx ry rz sx sy sz sw) */
{
  ...
  getNumArgFroma1((char *) data,10,GL_FLOAT);
...
Int gl_echelle() /* (GLscale sx sy sz fsize) NSUBR */
{
  ...
  if(!wobj || !getNumArgFroma1((char *) data,3,GL_FLOAT)) { a1=nil; derec; }
		    ...
		    if Open GL(GLnurbs nurbs liste-de-points-de-controle liste-de-points type) NSUBR */
{
  ...
  a1 = cdr(a1) ; pts_nb = getNumArgFroma1((char *)&points[0],0,GL_FLOAT);
...
Int gl_nurbs_surface() /* (GLsurface pts ctrl_pts type) SUBR3  */
{
  ...
  tmp = car(cdr(a1));
  a1 = car(a1);
  s_nk = getNumArgFroma1((char *) &points[0][0], 0, GL_FLOAT);
  if(tmp != nil) {
    a1 = tmp;
    t_nk = getNumArgFroma1((char *)&points[1][0], 0, GL_FLOAT);
    is_onek = 1;
  }
  else t_nk = s_nk;
